...
from new_utils.plotter import plot_matrix

from new_utils.diagnoser import Diagnoser
...
class SemanticAnalysis(Diagnoser):
    def __init__(self, annotations: pd.DataFrame, predictions: pd.DataFrame):
        ...
        self.text_encoder = CLIPTextModel.from_pretrained(self.clip_model).to(self.device)
        self.cossim = CosineSimilarity(dim=0, eps=1e-6)

    def analyze(self):
        confusion_matrix = self.compute_confusion()
        plot_matrix(confusion_matrix, 'Confusion Matrix', 'conf.png')
        ...
        plot_matrix(similarity_matrix, 'Similarity Matrix', 'sim.png')
    ...
    def load_classes(self):
        return self.annotations.label.unique().tolist()

    def dist(self, v1, v2):
        return self.cossim(v1, v2)    

    def tokenize(self, prompts):
        text_inputs = self.tokenizer(prompts, padding="max_length", return_tensors="pt").to(self.device)
        return torch.flatten(self.text_encoder(text_inputs.input_ids.to(self.device))['last_hidden_state'],1,-1)
    ...
    def compute_confusion(self):
        merged = pd.merge(self.annotations, self.predictions, on='video_id', how='inner')
        ...
        df = pd.DataFrame(confusion_matrix(y_test, y_pred, normalize='pred'), columns=list(class_map.keys()))
