...
from plotter import plot_matrix
...
class ActionDetectorDiagnosis(object):
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        self.inverse_activity_index = {value: key for (key, value) in activity_index.items()}
    ...
    def wrapper_compute_cm(self):
        confusion_matrix = Parallel(n_jobs=len(self.tiou_thresholds))(
                            delayed(compute_cm)(
                                prediction=self.prediction,
                                ground_truth=self.ground_truth,
                                tiou_thr=tiou_thr,
            ...
            if err_code:
                ...
                for cidx in self.activity_index.values():
                    ...
                    for tidx in range(len(self.tiou_thresholds)):
                        ...
                        tp = (~np.isnan(this_pred_df[self.matched_gt_id_cols[tidx]].T)).astype(int)
                        tp = tp[this_error_types!=err_code]
                        fp = np.abs(tp - 1)

                        # Computing prec-rec
                        this_tp = np.cumsum(tp).astype(float)
                        this_fp = np.cumsum(fp).astype(float)
                        rec = this_tp / npos
    ...
    def confuse(self):
        ...
        # Augment the prediction DataFrame with the error types
        label_list = list(self.inverse_activity_index.values())
        self.confusion_matrix = self.wrapper_compute_cm()[0]
        self.confusion_matrix = pd.DataFrame.from_dict(self.confusion_matrix)
        self.confusion_matrix.columns = label_list
        self.confusion_matrix["id"] = label_list
        self.confusion_matrix.set_index("id", drop=True, inplace=True)
        # self.confusion_matrix = self.confusion_matrix.div(self.confusion_matrix.sum(axis=1), axis=0)
        plot_matrix(self.confusion_matrix, "Confusion Matrix", "plots/conf_2.png")

        self.confusion_matrix.to_csv('out/confusion.csv')
...
def compute_cm(prediction,
    ...
    """Assumes that prediction is sorted by 'prediction-id' column """
    labels = list(ground_truth['label'].unique())
    confusion_matrix = {label: {_label: 0 for _label in labels} for label in labels}
    ...
    for idx, this_pred in this_prediction.iterrows():
        ...
        confusion_matrix[gt_with_max_tiou_label][this_pred_label] += 1
